...
public class MainController implements Initializable{
    ...
    @FXML
    private ScatterChart<Double, Double> signalChart3;
    ...
    @FXML
    private NumberAxis signalXAxis3;
    
    @FXML
    private NumberAxis signalYAxis3;
    ...
    @FXML
    private ComboBox<String> filtr;
    
    @FXML
    private ComboBox<String> filtrWindow;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		signalXAxis3.setAutoRanging(false);
		signalXAxis3.setLabel("t[s]");
		
		signalYAxis3.setAutoRanging(false);
		signalYAxis3.setLabel("A");

		
		
		histogramChart.setLegendVisible(false);
		signalChart.setLegendVisible(false);
		signalChart3.setLegendVisible(false);
		...
		ObservableList<String> possibleFiltr = FXCollections.observableArrayList(
				"Donloprzepustowy",
				"Pasmowy",
				"Górnoprzepustowy");
		
		filtr.setItems(possibleFiltr);
		filtr.setValue(possibleFiltr.get(0));
		
		ObservableList<String> possibleFiltrWindow = FXCollections.observableArrayList(
				"Prostokątne",
				"Hamminga",
				"Hanninga",
				"Blackmana");
		
		filtrWindow.setItems(possibleFiltrWindow);
		filtrWindow.setValue(possibleFiltrWindow.get(0));
	...
	private void arthmeticOperationAction(ActionEvent event) {
		...
		if(event.getSource() == plusButton) {
			...
			operation = "Mno¿enie";
		} else if(event.getSource() == weaveButton) {
			operation = "Splot";
	...
	@FXML
	private void runFiltr(ActionEvent e) {
		if(rowFilter.equals("")) {
			showError("Brak wartości wymaganej do filtrowania");
		}
		Integer valueInRowFilter = Integer.valueOf(rowFilter.getText());
		
		double[] listfiltervalues = SignalFilter.lowFilter(valueInRowFilter);
		
		switch (filtr.getValue()) {
			case "Pasmowy":{
				listfiltervalues =  SignalFilter.mediumFilter(listfiltervalues);
				break;
			}
			case "Górnoprzepustowy":{
				listfiltervalues =  SignalFilter.highFilter(listfiltervalues);
				break;
			}
			default:
				break;
		...
		switch (filtrWindow.getValue()) {
			case "Hamminga": {
				listfiltervalues = SignalFilter.hammingWindow(listfiltervalues);
				break;
			}
			case "Hanninga": {
				listfiltervalues = SignalFilter.hanningWindow(listfiltervalues);
				break;
			}
			case "Blackmana": {
				listfiltervalues = SignalFilter.blackmanWindow(listfiltervalues);
				break;
			}
			default: 
				break;
		...
		listfiltervalues = SignalFilter.countWeaveValuesWithoutSignal(signalsList.getSelectionModel().getSelectedItem().getValues(), listfiltervalues);
		drawFilterChart(listfiltervalues, signalsList.getSelectionModel().getSelectedItem());
	...
	private void showStatistics() {
		...
		amplitudeLabel.setText(String.format("%.2f", signal.getAmplitude()));
		...
		avgValueLabel.setText(String.format("%.2f",signal.getAvgValue()));
		absAvgValueLabel.setText(String.format("%.2f",signal.getAvgAbsValue()));
		avgPowerLabel.setText(String.format("%.2f",signal.getAvgPower()));
		varianceLabel.setText(String.format("%.2f",signal.getVariance()));
		effectiveValueLabel.setText(String.format("%.2f",signal.getEffectiveValue()));
	}
	...
	private void drawFilterChart(double[] listfiltervalues, Signal filteredSignal) {
		for (double d : listfiltervalues) {
			System.out.println(d);
		}
		signalChart3.getData().clear();
		int samplingFrequency = filteredSignal.getSamplingFrequency();
		int initTime = filteredSignal.getInitTime();
		
		signalXAxis3.setLowerBound(initTime - 1);
		signalXAxis3.setUpperBound(filteredSignal.getInitTime()  + filteredSignal.getDuration() + 1);
		signalYAxis3.setLowerBound(((int)Arrays.stream(listfiltervalues).min().getAsDouble()) - 2);
		signalYAxis3.setUpperBound(((int)Arrays.stream(listfiltervalues).max().getAsDouble()) + 2);
...
ObservableList<ScatterChart.Data<Double, Double>> data = FXCollections.observableArrayList();
		for(int i = 0; i < listfiltervalues.length; i++) {
			data.add(new ScatterChart.Data<Double, Double>(
					(i/(double)samplingFrequency) + initTime, listfiltervalues[i]));
		}
		
		ScatterChart.Series<Double, Double> series = new ScatterChart.Series<>(data);
		signalChart3.getData().add(series);
	}
